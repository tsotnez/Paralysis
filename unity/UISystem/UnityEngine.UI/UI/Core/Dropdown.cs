...
namespace UnityEngine.UI
{
    ...
    public class Dropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        protected internal class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler
        {
            ...
            [SerializeField]
            private bool m_SelectOnPointerEnter;
            ...
            public bool selectOnPointerEnter { get { return m_SelectOnPointerEnter; } set { m_SelectOnPointerEnter = value; } }
            ...
            public virtual void OnPointerEnter(PointerEventData eventData)
            {
                if (m_SelectOnPointerEnter)
                {
                    ...
                    if (dropdown != null)
                        dropdown.EventSystem.SetSelectedGameObject(gameObject);
                }
            }
        ...
        [Space]
        // additional fields to fix auto-hide, item-auto-select and select-on-hide behaviour Dropdown has by default
        [SerializeField]
        protected bool m_SelectSelfOnClose = true;
        [SerializeField]
        protected bool m_HideOnSelectItem = true;
        [SerializeField]
        protected bool m_SelectItemsOnPointerEnter = true;
        ...
        private void SetupTemplate()
        {
            ...
            item.selectOnPointerEnter = m_SelectItemsOnPointerEnter;
        ...
        public void Hide()
        {
            ...
            // reselect the dropdown only if desired 
            // it will make it hard for pure keyboard navigation to reuse it but pointer based input is causing blinking/re-pop-up issues on DropDown objects
            if (m_SelectSelfOnClose)
                Select();
        }
        ...
        private void OnSelectItem(Toggle toggle)
        {
            ...
            value = selectedIndex;
            // check flag if the dropdown should hide after selection
            if (m_HideOnSelectItem)
                Hide();
            else
            {
                // if not, reset the toggle values of all except the current selected
                for (int i = 0; i < m_Items.Count; i++)
                {
                    if (i != value)
                        m_Items[i].toggle.Set(false, false);
                    else
                        m_Items[i].toggle.Set(true, false);
                }
            }
        }
    }
